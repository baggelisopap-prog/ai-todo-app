...
/* ---------------------------------------------------------------------------
 * The four below exist for the task sheet's rows, where a drawing replaced the
 * uppercase caption that used to sit above every control. Drawn on the same
 * 24×24 grid at the same stroke weight as every other icon in this file — mixing grids is how one
 * icon ends up visibly heavier than its neighbour in the same column.
 *
 * They are never decorative: each one IS the label of its row, so every caller
 * gives it a real name (see FieldIcon in TaskDetailSheet).
 * ------------------------------------------------------------------------ */
...
export function FlagIcon({ className }) {
  // The pennant is FILLED with currentColor while the pole stays a stroke, so
  // the caller's priority colour reads as a block rather than an outline. That
  // fill is what carries the meaning: a flag shape says "priority" only to
  // somebody who already learned it, but red, amber and blue are the same
  // three colours this app uses for P1/P2/P3 everywhere else.
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
      <path d="M5 21V4" />
      <path d="M5 4h11l-2 3.5L16 11H5z" fill="currentColor" stroke="none" />
...
export function TextLinesIcon({ className }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
export function ReopenIcon({ className }) {
  // An arrow curving anticlockwise: "put this back the way it was". Used on the
  // handover notice, where the other action beside it is a plain ΟΚ — so this
  // one carries its word as well as its shape, and the shape only has to make
  // the word findable rather than carry the meaning alone.
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
