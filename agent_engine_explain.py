...
# =====================================================================
#
# ⚠️  ΠΡΟΣΟΧΗ — ΜΕΡΗ ΑΥΤΟΥ ΤΟΥ ΑΡΧΕΙΟΥ ΕΙΝΑΙ ΞΕΠΕΡΑΣΜΕΝΑ (20/09/2026)
# ---------------------------------------------------------------------
# Το αρχείο γράφτηκε στις 07/08/2026. Ο πραγματικός κώδικας συνέχισε να
# αλλάζει (τελευταία αλλαγή 17/09/2026) και το αντίγραφο έμεινε πίσω.
#
# Από τα 55 κομμάτια του πραγματικού κώδικα:
#   ✅  16  είναι σωστά εδώ
#   ❌  10  ΔΕΙΧΝΟΥΝ ΑΛΛΟ απ' ό,τι τρέχει στην πραγματικότητα
#   ❌  11  λείπουν εντελώς
#
# ΔΕΙΧΝΟΥΝ ΑΛΛΟ (μην τα εμπιστευτείς — δες τον πραγματικό κώδικα):
#   ask_agent, search_tasks, build_system_instruction, build_time_context,
#   build_tool_functions, build_write_proposal_tools, is_open_task,
#   propose_complete_task, propose_update_task, _finish
#
# ΛΕΙΠΟΥΝ ΕΝΤΕΛΩΣ:
#   is_disposed_of, render_task_rows, stem_words, build_vocabulary_block,
#   build_conversation_refs_block, _scan, _match_with, _unjustified_target,
#   STEM_MIN_WORD_LENGTH, STEM_PREFIX_LENGTH, _PENDING_ERROR
#
# ΓΙΑΤΙ ΜΕΝΕΙ ΕΤΣΙ ΚΑΙ ΔΕΝ ΔΙΟΡΘΩΘΗΚΕ ΑΜΕΣΩΣ: η πλήρης ενημέρωση σημαίνει
# να ξαναγραφτούν ~21 κομμάτια με σχόλιο ανά γραμμή — αρκετή δουλειά ώστε
# να αξίζει δική σου απόφαση, όχι να γίνει στο πέρασμα ενός καθαρίσματος.
# Μέχρι τότε, αυτή η λίστα είναι πιο χρήσιμη από ένα αρχείο που σου λέει
# ψέματα χωρίς να το ξέρεις.
#
# ΤΟ ΣΗΜΑΝΤΙΚΟ: δεν θα ξαναπαλιώσει ΣΙΩΠΗΛΑ. Το
# tests/test_explain_copy_is_current.py συγκρίνει αυτό το αρχείο με τον
# πραγματικό κώδικα σε κάθε τρέξιμο των τεστ και κοκκινίζει αν ανοίξει νέα
# διαφορά. Όταν ενημερωθεί ένα κομμάτι, σβήνεται και από εκεί.
# ---------------------------------------------------------------------
#
