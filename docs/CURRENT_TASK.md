...
## What it does
An add through the + — typed, dictated or photographed — switches to the Inbox, and the new cards take an amber ring with a halo that pulses three times (~1,2 s each) and then stops, leaving the steady ring behind. The ring comes off a card when that card is opened (only that one — an extraction can return several), and off everything after 12 seconds if nothing is touched. A second add restarts the clock rather than letting the older timer cut the newer mark short.
